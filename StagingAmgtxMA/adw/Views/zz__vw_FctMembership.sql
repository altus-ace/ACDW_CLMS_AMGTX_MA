...
CREATE VIEW [adw].[zz__vw_FctMembership]
AS
SELECT
...
FROM adw.[FctMembership_Dev] FMbr
	...
	JOIN (SELECT pr.NPI NPI, pr.AttribTIN  TIN, pr.FirstName AS FirstName
			 ...
			 FROM (SELECT * FROM (
						SELECT *
							FROM [ACECAREDW].adw.tvf_AllClient_ProviderRoster (21,'2021-04-15',1) pr
									JOIN (	SELECT * 
												FROM (
