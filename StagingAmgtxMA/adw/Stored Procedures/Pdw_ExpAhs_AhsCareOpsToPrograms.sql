...
CREATE ProcedURE [adw].[Pdw_ExpAhs_AhsCareOpsToPrograms](    
    @QmBatchDate DATE, 
    @ClientKey INT
)
AS
BEGIN
    --DECLARE @QmBatchDate DATE = '11/15/2021'  
      ...
      INSERT INTO adw.AhsExpCareOpsToPrograms (LoadDate, CareOpBatchDate, SrcKey, SrcTableName, ClientKey, ClientMemberKey
        ...
        SELECT @QmBatchDate AS LoadDate, @QmBatchDate  AS CareOpBatchDate
    	   , cg.SrcKey, cg.SrcTableName, cg.ClientKey, CG.ClientMemberKey
        	   ...
        	   , CASE WHEN (cg.QmDate < DATEFROMPARTS(YEAR(@QmBatchDate), 1, 1)) THEN  DATEFROMPARTS(Year(@QmBatchDate), 1, 1) 
					ELSE DATEFROMPARTS(Year(CG.QmDate), Month(CG.QMDate), 1) END AS StartDate	
        	   , CASE WHEN (cg.QmDate < DATEFROMPARTS(YEAR(@QmBatchDate), 1, 1)) THEN  DATEFROMPARTS(Year(@QmBatchDate), 1, 1) 
					ELSE DATEFROMPARTS(Year(CG.QmDate), Month(CG.QMDate), 1) END AS CreateDate
        	   , DATEFROMPARTS(Year(@QmBatchDate), 12, 31) AS StopDate
            ...
            FROM ast.QmToProgCurGaps CG
        	   ...
        	   LEFT JOIN ACECAREDW.dbo.TmpAllMemberMonths mbr 
        	   ...
        	   JOIN lst.lstMapCareoppsPrograms CoProg 
				...
				and CG.ClientKey = @ClientKey        			 
        			 AND @QmBatchDate BETWEEN CoProg.EffectiveDate and CoProg.ExpirationDate    		  
		  LEFT JOIN (SELECT cop.ClientKey, cop.ClientMemberKey, cop.QmMsrId, cop.CalcQmCntCat, cop.QMDate
						...
						WHERE @QmBatchDate BETWEEN cop.programStartDate and cop.ProgramEndDate) adw 
			...
			LEFT JOIN (SELECT co.CLIENT_PATIENT_ID,   co.LOB, co.PROGRAM_NAME, co.START_DATE, co.end_date, co.plan_start_date, co.plan_end_date, co.PROGRAM_STATUS_NAME	  
						FROM Ahs_Altus_Prod.dbo.vw_ACE_ALT_PE co
							JOIN lst.List_Client Client ON co.LOB = Client.CS_Export_LobName
						WHERE YEAR(co.START_DATE) >= YEAR(dateAdd(Year, -1, GETDATE()) ) 
							AND co.PROGRAM_STATUS_NAME <> 'TERM'
							and @ClientKey = Client.ClientKey) AHS 
					ON CG.ClientMemberKey = AHS.CLIENT_PATIENT_ID
						and CoProg.DESTINATION_PROGRAM_NAME = AHS.PROGRAM_NAME
						and DATEFROMPARTS(Year(CG.QmDate), Month(CG.QMDate), 1) = AHS.START_DATE
						and AHS.end_date > @QmBatchDate
            WHERE CG.SendFlg = 1								-- Set to Export Row
        		  AND ISNULL(mbr.ClientMemberKey, '') != ''		-- IS the Active
    		  AND adw.ClientMemberKey is null	-- Member does not have this program for this month
			  AND CoProg.ACTIVE = 'Y'			-- Program is active 
			  AND CG.RecStatus IN ('E','N')		-- PRogram is still open
    END;	
END;
