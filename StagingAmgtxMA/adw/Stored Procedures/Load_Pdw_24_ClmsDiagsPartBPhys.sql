...
AS -- insert claims diags for Steward_MSSPPartBPhysicianClaimLineItem
   ...
   SELECT COUNT(*) 
    ...
    SELECT	   dg.ClaimNbr				AS SEQ_CLAIM_ID   
			 ...
			 , dg.DIAG_CD					AS DiagCode
			 , ''				     AS DiagPoa
			 , getDate()			     AS LoadDate
			 , '[ACDW_CLMS_AMGTX_MA].[adi].[Amerigroup_CLM_DIAG]' AS SrcAdiTableName
			 , ast.DiagAdiKey			as SrcAdiKey			 
    FROM ast.Claim_06_Diag_Dedup ast
	   JOIN [ACDW_CLMS_AMGTX_MA].[adi].[Amerigroup_CLM_DIAG] dg ON ast.DiagAdiKey = dg.Amerigroup_CLM_DIAGKey
	   And SrcClaimType = 'PROF'
	    JOIN [ACDW_CLMS_AMGTX_MA].[adi].[Amerigroup_MedClaimDetl] cd
		  ON cd.ClaimNbr = dg.ClaimNbr 
