...
AS 
	...
	FROM [adw].FctMembership mbr
	    ...
	    INNER JOIN (SELECT mbr.ClientMemberKey, MAX(mbr.RwEffectiveDate) mbrRwEff, Max(mbr.RwExpirationDate) MbrRwExp from Adw.FctMembership mbr GROUP BY mbr.ClientMemberKey) LatestRecords
			ON mbr.ClientMemberKey = LatestRecords.ClientMemberKey
				and mbr.RwEffectiveDate = LatestRecords.mbrRwEff
				and mbr.RwExpirationDate = LatestRecords.MbrRwExp
				and MBR.ClientKey =@ClientKey    
